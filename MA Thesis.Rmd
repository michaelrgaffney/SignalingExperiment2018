...
demovars <-
  c(
    ...
    "MC2.4_1" = "How easy to imagine",
    "Angry" = "Is sister angry?",
    "Sad" = "Is sister sad?",
    "Suicidal" = "Is sister suicidal?",
    "MentallyIll" = "Is sister mentally ill?",
    "Depressed" = "Is sister depressed?",
    "OutOfOptions" = "Is sister out of options?",
    "Happy" = "Is sister happy?",
    "Neutral" = "Does sister feel neutral?",
    "Scared" = "Is sister scared?",
    "Tired" = "Is sister tired?",
    "Distressed" = "Is sister distressed?",
    "NoneOfAbove" = "None of the above"
  )
options(scipen=999)
kable(
  custom.summarize(d0, demovars),
  caption = 'Table 2a. Summary statistics for variables measured at time 2 only. Includes only participants who met our inclusion criteria described above. A breakdown of the number of participants by state is availible in Appendix 1.'
)
```
...
kable(
  df_t1t2_sum,
  caption = "Table 2b. Summary statistics for variables measured at both time 1 and time 2. Includes only participants who met our inclusion criteria described above."
  )
```
...
```{r pcaT1Loadings, fig.cap= "Figure 1. Loadings plot showing how each of the variables included in our T1 PCA load on the first two principle components."}
p_pcaT1Loadings <- pca_loadings_plot(pcaT1, components = 1:2)
ggsave("Figures/Figure1.pdf", p_pcaT1Loadings, width = 9, height = 6)
p_pcaT1Loadings
```


```{r biplotT1, fig.cap= "Figure 2. Biplot of PC1 and PC2 at T1. The arrows represent the loadings of each variable on the first two prinicpal components. Each dot is one participant."}
biplotT1
ggsave("Figures/Figure2.pdf", biplotT1, width = 9, height = 6)
```
...
```{r manipulations, fig.cap="Figure 3. Manipulation checks: the effects of conflict and information conditions on reactions to helping the sister. See Appendix 3 for regression table."}
pT1a + pT1b
ggsave("Figures/Figure3.pdf", pT1a + pT1b, width = 9, height = 6)
```

```{r T1Anova}
kable(tidy(Anova(mT1manipulation)), digits = 1, caption="Table 3. Anova of linear regression of PC1 as a function of the conflict and information conditions.")
```
...
In order to see if our signal manipulations resulted in the desired perceptions (I.E. the depressed manipulation resulted in perceptions of depression), we examined the percent of participants that checked each box of our periceved emotional state manipulaton check sorted by the signal they saw (Visualized in Figure 4). In doing so it appears that our signal manipulations were largely identified correctly, with $`r signif(signalmat[7,3] * 100, 2)`$% and $`r signif(signalmat[5,3] * 100, 2)`$% of participants reporting the sister as suicidial in the suicide attempt condition and the depression plus display of suicidial ideation respectively. Similarly, $`r signif(signalmat[4,5] * 100, 2)`$% of participants in the depressed condition viewed the sister as depressed, and $`r signif(signalmat[6,4] * 100, 2)`$% participants in the schizophrenic condition viewed the sister as mentally ill.


```{r signalheatmap, fig.cap=" Figure 4. How participants in each signal condition rated the sister's mental state. Participants could check any number of conditions. Colors represent the percent of participants that checked each condition. Red = low, Brown = middle, Yellow and White = high."}
heatmap(signalmat, scale = 'none')
pdf(file = "Figures/Figure4.pdf", width = 9)
heatmap(signalmat, scale = 'none', margins = c(8,5))
dev.off()
```
...
To better understand which combinations of responses were most meaningufl, we also ran a PCA on all the variables that recorded if participants percieved the imagined sister to be in the emotional states listed above. PC1 seems to capture perceptions of sadness, fear, depression, and distress (Figure 5), while PC2 is heavily influenced by perceptions of mental illness (Figure 6). See Appendix 4 for the loadings plot and biplot. 

```{r epc1, fig.cap= "Figure 5. Boxplot showing the loadings on PC1 for each of the signals for our second PCA looking at perceptions of the sister's emotional state."}
epc1
ggsave("Figures/Figure5.pdf", epc1, width = 9, height = 6)
```


```{r mentallyill, fig.cap= "Figure 6. Plot showing the proportion of participants who percieved their imagined sister to be mentally ill. Perceptions of mentall illness is plotted on the x-axis instead of PC2 due to both PC2 appearing to represent primariliy percpetions of mentall illness and because perceptions of mental illness are the more theoretically relvant variable."}
p_mentallyill
ggsave("Figures/Figure6.pdf", p_mentallyill, width = 9, height = 6)
```
...
We also fit linear models for the same outcome variables to examine test the hypothesized signal by conflict interaction. Unlike the predicted signal and information interactions, 3 of the 4 models showed evidence of interactions with the model for change in likelihood of lending T2 being marginally significant ($p = `r signif(int2sig['signal2:conflict', 'Pr(>F)'], 2)`$). See Figure 9 for effects plot and Appendix 7 for the corresponding ANOVA tables.

```{r conflictInteractions, fig.cap="Figure 9. Signal and conflict interactions. The effect of the signal, conflict interaction on changes in (A) the amount of money the participant is comfortable lending the sister at T2, controlling for the amount at T1 (dotted line indicates mean amount at T1), (B) the likelihood of lending the sister money, (C) PC1 of all the outcome variables at T2 controlling for T1, and (D) participants' perception of the sister's need for money.  Effects are averaged across all time 1 conditions."}
p_conflictInteractions <- (p_comfort_signal_conflict$plot + p_lend_signal_conflict$plot + scale_y_continuous(limits = c(-40, 20)))/(p_pc1_signal_conflict$plot + p_money_signal_conflict$plot + scale_y_continuous(limits = c(-40, 20)))

p_conflictInteractions

ggsave("Figures/Figure9.pdf", p_conflictInteractions, width = 9, height = 9)
```

See Appendix 8 for a full overview of the full 2x3 interaction with change in perceived need at time 2 as the outcome variable.
...
```{r pc1t1t2, fig.cap=" Figure 10. PC1 at time 2 as a function of PC1 at time 1, by signal and conflict condition. Each dot is one participant. Lines fit by loess regression."}
p_pc1_t1t2
ggsave("Figures/Figure10.pdf", p_pc1_t1t2, width = 9, height = 6)
```
...
```{r exploratory model, fig.cap= "Figure 11. Effects plot showing the interaction between signal type and conflict in our favored exploratory model."}
Plot_Exploratory
ggsave("Figures/Figure11.pdf", Plot_Exploratory, width = 9, height = 9)
```

```{r expanova}
kable(tidy(Anova(m_exp, type = 3)), digits = 1, caption="Table 4. Anova of interaction model depicted in Figure X.")
```

### Mediation model

As changes in perceived need were strongly correlated with our measures of instrumental support, we fit multiple mediation models to see how this was influenced by signal type using the ‘mediation’ package in R [@ImaiEtAl2010]. See Figure 12.

```{r mediationmodel, fig.cap="Figure 12. Mediation model of causal effect of depression signal on change in likelihood of lending money, relative to verbal control."}
grViz(
  "digraph {
...
a [pos = '-2, 0!', label = 'Signal\n(Depression vs Verbal)']
b [pos = '0, 1!', label = 'ΔNeeds money']
c [pos = '2, 0!', label = 'ΔLend money']
...
Our first model compared the depression to the verbal reuqest condition. According to this analysis, the total effect of the depression signal was to increase the likelihood of lending money by `r signif(mediation_model$tau.coef, 3)` points, relative to a verbal request; `r 100*signif(mediation_model$n0, 2)`% of this effect was mediated by the increased perception that the sister needed money. See Figure 13.

```{r mediation plot, fig.cap= "Figure 13. Mediation of signal effect on likelihood of lending money by change in percieved need (controlling for percieved need at time 1). ACME is the average causal mediation effect; ADE is the average direct effect."}
pdf("Figures/Figure13.pdf", width = 9, height = 6)
plot(mediation_model)
dev.off()
```

In the model comparing depression to crying, the direct effect of the depression signal was to decrease the likelihood of lending money by `r signif(mediation_model2$z.avg * -1, 3)` points relative to a verbal request, while the mediated effect increased the liklihood of lending money by `r signif(mediation_model2$d.avg, 3)` points. The total effect of depresion relative to crying was an increase of `r signif(mediation_model2$tau.coef, 3)` points.

Lastly, in the model comparing depression to the control, the direct effect of the depression signal was to decrease the likelihood of lending money by `r signif(mediation_model3$z.avg * -1, 3)` points relative to a verbal request, while the mediated effect increased the liklihood of lending money by `r signif(mediation_model3$d.avg, 3)` points. The total effect of depresion relative to the control was an increase of `r signif(mediation_model3$tau.coef, 3)` points.

Unlike the depression and verbal request model, the proportion of the relationship that was mediated by changes to perception of need was not reported for the follwoing models due to the changes in sign between the direct and mediated effects making such estimates meaningless [@HayesRockwood2017]
...
The final area where our main effects predictions were not borne out regarded the anger condition. As aggression and depression have been suggested to be alternative ways to accomplish similar bargaining outcomes [@HagenRosenstrom2016], we predicted anger to have a similar effect to depression on helping behavior. However, anger resulted in less help in both measures. Unfortunately, the fact that the signaler was always a woman means that this study is an insufficient test of this hypothesis as formidability is strongly confounded with sex [@SellEtAl2009]. Therefore, the results of the anger condition may be driven by displays of anger being a less effective signal in women as they are unlikely to bargain as well through aggression. In addition to this, the likelihood that anger also serves as a way to downregulate one’s welfare tradeoff ratio with another further limits the ability of this study to evaluate anger as a signal of aggression as this was not controlled for [@SellEtAl2009]. 
...
```{r numbersbystate, fig.height=10, fig.cap= "Numbers of participants by state. Note the east coast bias caused by the study being completed much sooner than expected."}
pdf("Figures/Appendix1.pdf", width = 9, height = 12)
...
```{r pcaT1, fig.cap= "Table showing the loadings for our PCA at time 1 for both PCA1 and PCA2."}
mat <- pcaT1$rotation[,1:2]
kable(
  mat, 
  digits = 2,
  caption = 'Variable loadings for PCA of  T1 support and emotion variables'
  )
```

## Appendix 3

```{r, mT1manipulationSummary}

kable(tidy(mT1manipulation), digits = 2, caption = 'Coefficients of the linear regression model of PCA1 T1 as a function of information and conflict.')

kable(glance(mT1manipulation), caption = 'Table showing additional summary statistics for the model.')
```

## Appendix 4

```{r emotionPCAloadings, fig.cap= "Loadings plot showing how each of the variables included in our sister's emotions PCA load on the first three principle components."}
pca_loadings_plot(Epca)#, components = 1:2)
ggsave("Figures/Appendix6.pdf", pca_loadings_plot(Epca), width = 9, height = 6)
```

```{r emotionPCAbiplot, fig.cap= "Biplot of PC1 and PC2 for the sister's percieved emotional states. The arrows represent the loadings of each variable on the first two prinicpal components. Each dot is one participant."}
emotions_biplot
```

## Appendix 5

Regression statistics and ANOVAs of our linear regressions for our four outcome variables as a function of signaling condition.

### 5A Amount comfortable lending T2 (controlling for T1)

```{r signalA}
kable(tidy(mT2comfort), digits = 2, caption = 'Coefficients of the linear regression model of the amount of money comfortable lending T2 (controlling for T1) as a function of signal manipulation.')

kable(glance(mT2comfort), caption = 'Table showing additional summary statistics for the model.')
```

### 5B Change in likelihood of lending money T2

```{r signalB}
kable(tidy(mT2lend), digits = 2, caption = 'Coefficients of the linear regression model of the change in likelihood participants would lend the money at T2 as a function of signal manipulation.')

kable(glance(mT2lend), caption = 'Table showing additional summary statistics for the model.')
```

### 5C Change in percieved need T2

```{r signalC}
kable(tidy(mT2need), digits = 2, caption = 'Coefficients of the linear regression model of change in percieved need T2 as a function of signal manipulation.')

kable(glance(mT2need), caption = 'Table showing additional summary statistics for the model.')
```

### 5D PC1 T2 (Controlling for PC1 at T1)

```{r signalD}
kable(tidy(mT2pc1), digits = 2, caption = 'Coefficients of the linear regression model of PC1 T2 (controlling for T1) as a function of signal manipulation.')

kable(glance(mT2pc1), caption = 'Table showing additional summary statistics for the model.')
```
...
### 6A Amount comfortable lending T2 (controlling for T1)

```{r signalinfA}
kable(tidy(p_comfort_signal_pinfo$model), digits = 2, caption = 'Coefficients of a linear regression model of amount comfortable lending T2 (controlling for T1) as a function of signaling condition, information, and their interaction.')

kable(glance(p_comfort_signal_pinfo$model), caption = 'Table showing additional summary statistics for the model.')

kable(tidy(Anova(p_comfort_signal_pinfo$model, type = 3)), digits = 1, caption = 'ANOVA of model with amount comfortable lending T2 (controlling for T1) as the outcome variable.')

```

### 6B Change in likelihood of lending money T2

```{r signalinfB}

kable(tidy(p_lend_signal_pinfo$model), digits = 2, caption = 'Coefficients of a linear regression model of change in likelihood of lending as a function of signaling condition, information, and their interaction.')

kable(glance(p_lend_signal_pinfo$model), caption = 'Table showing additional summary statistics for the model.')

kable(tidy(Anova(p_lend_signal_pinfo$model, type = 3)), digits = 1, caption = 'ANOVA of model with change in likelihood of lending T2 as the outcome variable.')


```

### 6C PC1 T2 (Controlling for PC1 at T1)

```{r signalinfC}
kable(tidy(p_pc1_signal_pinfo$model), digits = 2, caption = 'Coefficients of a linear regression model of PC1 as a function of signaling condition, information, and their interaction, controlling for T1.')

kable(glance(p_pc1_signal_pinfo$model), caption = 'Table showing additional summary statistics for the model.')

kable(tidy(Anova(p_pc1_signal_pinfo$model, type = 3)), digits = 1, caption = 'ANOVA of model with PC1 as the outcome variable, controlling for t1.')


```

### 6D Change in percieved need T2

```{r signalinfD}
kable(tidy(p_money_signal_pinfo$model), digits = 2, caption = 'Coefficients of a linear regression model of change in perceived need of money as a function of signaling condition, information, and their interaction.')

kable(glance(p_money_signal_pinfo$model), caption = 'Table showing additional summary statistics for the model.')

kable(tidy(Anova(p_money_signal_pinfo$model, type = 3)), digits = 1, caption = 'ANOVA of model with change in percieved need of money as the outcome variable.')
...
### 7A Amount comfortable lending T2 (controlling for T1)

```{r signalconfA}
kable(tidy(p_comfort_signal_conflict$model), digits = 2, caption = 'Coefficients of a linear regression model of amount comfortable lending (controlling for T1) as a function of signaling condition, information, and their interaction.')

kable(glance(p_comfort_signal_conflict$model), caption = 'Table showing additional summary statistics for the model.')

kable(tidy(Anova(p_comfort_signal_conflict$model, type = 3)), digits = 1, caption = 'ANOVA of model with amount comfortable lending T2 (controlling for T1) as the outcome variable.')

```

### 7B Change in likelihood of lending money

```{r signalconfB}
kable(tidy(p_lend_signal_conflict$model), digits = 2, caption = 'Coefficients of a linear regression model of change in likelihood of lending as a function of signaling condition, information, and their interaction.')

kable(glance(p_lend_signal_conflict$model), caption = 'Table showing additional summary statistics for the model.')

kable(tidy(Anova(p_lend_signal_conflict$model, type = 3)), digits = 1, caption = 'ANOVA of model with change in likelihood of lending T2 as the outcome variable.')


```

### 7C PC1 T2 (Controlling for PC1 at T1)

```{r signalconfC}
kable(tidy(p_pc1_signal_conflict$model), digits = 2, caption = 'Coefficients of a linear regression model of PC1 T2 (controlling for T1) as a function of signaling condition, information, and their interaction.')

kable(glance(p_pc1_signal_conflict$model), caption = 'Table showing additional summary statistics for the model.')

kable(tidy(Anova(p_pc1_signal_conflict$model, type = 3)), digits = 1, caption = 'ANOVA of model with PC1 as the outcome variable, controlling for t1.')


```

### 7D Change in percieved need

```{r signalconfD}
kable(tidy(p_money_signal_conflict$model), digits = 2, caption = 'Coefficients of a linear regression model of change in perceived need of money as a function of signaling condition, information, and their interaction.')

kable(glance(p_money_signal_conflict$model), caption = 'Table showing additional summary statistics for the model.')

kable(tidy(Anova(p_money_signal_conflict$model, type = 3)), digits = 1, caption = 'ANOVA of model with change in percieved need of money as the outcome variable.')
```

## Appendix 8

```{r Full Interaction Overview, fig.cap= "Effects plots showing changes in perceived need from T1 to T2 as a function of the interaction between the information and conflict manipulations. The y-axis is ordered based on our predictions, with signals expected to result in more support as one moves from bottom to top."}
# plot(allEffects(full_int_overview2))
# Plot_full_int
p_overview
ggsave("Figures/Appendix5.pdf", p_overview, width = 9, height = 6)
```

## Appendix 9

```{r ease of putting oneself in the scenario, fig.cap= "Effects plot showing how easily participants percieved imagining themselves in this situation based on manipulation seen as a function of: (A) signal by information, and (B) signal by conflict. Higher values indicate greater ease (0 = Extremely difficult; 100 = Extremely easy)." }
p_ease$plot$layers[[3]] <- NULL
p_ease2$plot$layers[[3]] <- NULL
p_ease$plot + p_ease2$plot + theme(axis.text.y=element_blank())
ggsave("Figures/Appendix7.pdf", p_ease$plot + p_ease2$plot + theme(axis.text.y=element_blank()), width = 9, height = 6)
```

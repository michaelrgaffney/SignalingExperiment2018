...
demovars <-
  c(
    ...
    "needsmoneyt1" = "Perceived need of money at T1",
    "likelylendmoneyt1" = "How likely to lend money at T1",
    "angryt1" = "How angry are you about the request at T1",
    "satisfactiont1" = "How satisfied to help at T1",
    "howsadt1" = "How sad about health problem at T1",
    "howreasonablet1" = "How reasonable is the amount requested at T1",
    "believehealtht1" = "How much do believe the niece is ill at T1",
    "daughterharmt1" = "How much harm will your daughter suffer at T1",
    "believeneedt1" = "Suspicion of sister at T1",
    "sisterbenefitt1" = "Perceived benefit to sister at T1",
    "trustrepayt1" = "Trust that sister will repay at T1",
    "comfortablelendingt1" = "Amount conformtable lending at T1 in dollars",
    "MC1.1_1" = "How close to sister at T1",
     
    "delta_needs_money" = "Change in perceived need of money at T2",
    "likelylendmoneyt2" = "Change in how likely to lend money at T2",
    "angryt2" = "Change in anger about the request at T2",
    "satisfactiont2" = "Change in How satisfied to help at T2",
    "howsadt2" = "Change in How sad about health problem at T2",
    "howreasonablet2" = "Change in How reasonable is the amount requested at T2",
    "believehealtht2" = "Change in How much do believe the niece is ill at T2",
    "daughterharmt2" = "Change in How much harm will your daughter suffer at T2",
    "believeneedt2" = "Change in Suspicion of sister at T2",
    "sisterbenefitt2" = "Change in Perceived benefit to sister at T2",
    "trustrepayt2" = "Change in Trust that sister will repay at T2",
    "comfortablelendingt2" = "Change in Amount conformtable lending at T2 in dollars",
    "MC1.2_1" = "How close to sister at T2",
...
kable(
  ...
  caption = 'Table 2. Summary statistics for all participants who met our inclusion criteria described above. A breakdown of the number of participants by state is availible in Appendix 1.'
)
...
pca_loadings_plot(pcaT1, components = 1:2)
```
...
Like with the main effects of signal type, we fit linear models for our four outcome variables: (1) the amount of money the participant is comfortable lending the sister at T2, controlling for the amount at T1, (2) the likelihood of lending the sister money, (3) PC1 of all the outcome variables at T2, and (4) participants' perception of the sister's need for money. Our prediction was only supported when looking at our model with the amount of money participants were comfortable lending at T2 (controlling for T1) ($p = `r signif(int1sig['signal2:p_info', 'Pr(>F)'], 2)`$. See Figure 8 for visualization and Appendix 3 for the corresponding ANOVA tables.
...
(p_comfort_signal_pinfo$plot + p_lend_signal_pinfo$plot + scale_y_continuous(limits = c(-40, 20)))/(p_pc1_signal_pinfo$plot + p_money_signal_pinfo$plot + scale_y_continuous(limits = c(-40, 20)))
```
...
(p_comfort_signal_conflict$plot + p_lend_signal_conflict$plot + scale_y_continuous(limits = c(-40, 20)))/(p_pc1_signal_conflict$plot + p_money_signal_conflict$plot + scale_y_continuous(limits = c(-40, 20)))
```
...
As changes in perceived need were strongly correlated with our measures of instrumental support, we fit a mediation model to see how this was influenced by signal type using the ‘mediation’ package in R [@ImaiEtAl2010]. See Figure 12.
...
Anovas of linear regressions of for the signal and information interactions for our four outcome variables.
...
kable(tidy(Anova(p_pc1_signal_pinfo$model, type = 3)), digits = 1, caption = 'ANOVA of model with PC1 as the outcome variable.')
...
Anovas of linear regressions of for the signal and conflict interactions for our four outcome variables.
...
kable(tidy(Anova(p_pc1_signal_conflict$model, type = 3)), digits = 1, caption = 'ANOVA of model with PC1 as the outcome variable.')
...
```{r emotionPCAloadings, fig.cap= "Loadings plot showing how each of the variables included in our sister's emotions PCA load on the first two principle components."}
